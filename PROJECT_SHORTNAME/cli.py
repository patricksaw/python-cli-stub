"""This module provides the PROJECT_FULLNAME CLI."""
...
from PROJECT_SHORTNAME import ERRORS, __app_name__, __version__, config, database, PROJECT_SHORTNAME
...
def get_todoer() -> PROJECT_SHORTNAME.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "PROJECT_SHORTNAME init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return PROJECT_SHORTNAME.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "PROJECT_SHORTNAME init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
